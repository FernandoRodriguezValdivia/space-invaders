...
class Enemy {
  ...
  update(x, y){
    ...
    this.game.projectilesPool.forEach(projectile => {
      if(!projectile.free && this.game.checkCollision(projectile,this) && this.lives > 0){
        this.hit(1);
        projectile.reset();
      }
    })
...
class Boss {
  ...
  update(){
    ...
    if(this.game.checkCollision(this, this.game.player) &&  this. lives >= 1 && this.y >= 0){
      this.game.gameOver = true;
      this.lives = 0;
    }
    // boss destroyed
...
class Game {
  constructor(canvas){
    ...
    window.addEventListener('keydown', (e) => {
      ...
      if(e.key === 'e' && this.tutorial) this.tutorial = false;
    })
  ...
  drawStatusText(context){
    ...
    if(this.gameOver){
      ...
      context.fillText('Preciona R para Reiniciar', this.width * 0.5, this.height * 0.5 + 40);
    }
    context.restore();
  }
  drawTutorial(context){
    ...
    context.fillText('Instrucciones', this.width * 0.5, this.height * 0.3);
    context.font = '30px Impact';
    context.fillText('Presiona 1 para proyectiles normales', this.width * 0.5, this.height * 0.3 + 80);
    context.fillText('Presiona 2 para láser nivel 1', this.width * 0.5, this.height * 0.3 + 130);
    context.fillText('Presiona 3 para láser nivel 2', this.width * 0.5, this.height * 0.3 + 180);
    context.fillText('Al usar el láser, la barra de energia no debe', this.width * 0.5, this.height * 0.3 + 230);
    context.fillText('llegar a cero', this.width * 0.5, this.height * 0.3 + 280);
    context.fillText('Presiona E para empezar', this.width * 0.5, this.height * 0.3 + 330);
    context.restore();
  }
  newWave(){
    this.waveCount++;
    ...
    this.wave = this.wave.filter(obj => !obj.markedForDeletion);
  }
  restart(){
    this.player.restart();
    this.columns = 2;
